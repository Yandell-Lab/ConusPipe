...
my $usage = "discoveryPipeline.pl   <configure file>\n
...
close OUT9;
#%sigDVal=();
##use blastp on the tri.no_anot.pep.potentialNewToxAfterSigP.fa, after filter on: abundant (> 100 tpm), putative conotoxins of new superfamily will be reported only when there are 2 or more species in one hit each other. remove the bp hits from %sigPIdSeq, the rest seq in the data structure will be stored in arrays for logistic regression.
...
my(@testConoNocono,@testCysP,@testDVal,@testLen,%forNumId,%uniqBpId);#{numId}->seqId
my$numId=1;
...
foreach my $sid (keys %sigPIdSeq){
	if(!defined $uniqBpId{$sid}){
		my$pepLen=length($sigPIdSeq{$sid}->[3]);
		push @testLen, $pepLen;
		...
		my$cysPct=$cysNum/$pepLen*100;
		push @testCysP,$cysPct;
		push @testDVal,$sigDVal{$sid};
	}
} 
%sigDVal=();
...
# filter on: abundant (> ?? tpm), high prob (> 0.6 probability) peptides; for tpm,nt&pep seq: $sigPIdSeq{$seqId}->[len,tpm,ntSeq,pepSeq] 
	if(($prob[10]>=$probCutoff) && ($sigPIdSeq{$seqId}->[1] >= $tpmCutoff)){
		...
		my $header2=">$seqId\ttpm:$sigPIdSeq{$seqId}->[1]\tlen:$sigPIdSeq{$seqId}->[0]\tprobability:$prob[10]\n";
