...
Description:
This is version 8:
input xx.no_anot.??.fa should be the output of different species(sp and frame should be labeled in the id) from previous conotoxin annotation pipeline (tABRA.v2.pl) - xx.tri.no_anot.fa should have tpm value on the header, triNoAnotPepFa should be the original 6 frame translated pep seq directly from nonHits nt seq, with * inside of pepseq.
