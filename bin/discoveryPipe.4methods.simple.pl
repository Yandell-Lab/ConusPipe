...
my $usage = "discoveryPipeline.pl  [-options] conotoxinseq noConotoxinseq fpDb  lengthLowCutoff lengthCutoff evalCutoff DvalCutoff  codeCharMwPi outName\n
Options:
--nucleotide <nt.fa>
--peptide <pep.fa>

Description:
This is simple version which does not require tpm value for input seq:
input peptide or nucleotide cDNA seq -- opitons.
...
my($conoSeq,$NoconoSeq,$fpDb,$lengthLowCutoff,$lengthCutoff,$evalCutoff,$DvalCutoff,$aaInfor,$sample)=@ARGV;
...
#blastp against known false positives
system("blastp -db $fpDb -query $triNoAnotPepFa -num_threads 20 -outfmt '6 std qframe sframe' -evalue $evalCutoff -matrix BLOSUM62 -max_target_seqs 1 -out $sample.vs.fp.bp.out");
my($fpBpOut)=<$sample.vs.fp.bp.out>;
my$fpBpHash=getHit($fpBpOut);
my %sigPNumId; #{numId}->seqId 
my$idCount=1;
open OUT8,">$sample.tri.no_anot.pep.potentialNewToxBeforeSigP.fa";

#extract seq from M to stop codon which is longer than 5lowCutoff aas but shorter than seqlength upper limit
...
while (defined (my$line1=<FH1>)){
	chomp $line1;
		...
		next if ($mIndex==-1 || defined $fpBpHash->{$pepId});
